...
for (g in scraped_games) {
  ...
  game <- list()
  game$home_moneyline <- html_attr(g, 'data-game-odd')
  game$date <- html_attr(g, 'data-game-date')
  game$conference <-  html_attr(g, 'data-conference')
  game$home_team <-   html_attr(g, 'data-home-team-shortname-search')
  game$away_team <-   html_attr(g, 'data-away-team-shortname-search')
    
  tryCatch({
    game$home_score <- html_nodes(g, 'div.cmg_matchup_list_score_home') %>% html_text() %>% trimws()
    game$away_score <- html_nodes(g, 'div.cmg_matchup_list_score_away') %>% html_text() %>% trimws()
  }, error = function(e) {
    game$home_score <- ''
    game$away_score <- ''
  })

  game_data <- c(game_data, list(game))
}
